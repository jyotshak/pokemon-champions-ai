...
__export(trivia_exports, {
  ...
  Trivia: () => Trivia,
  altMergeTimeouts: () => altMergeTimeouts,
...
const MERGESCORE_EXPIRY = 15 * 60 * 1e3;
...
const altMergeTimeouts = /* @__PURE__ */ new Map();
  ...
  async mergescore(target, room, user) {
    ...
    try {
      ...
      return this.sendReply(
        `A Trivia leaderboard score merge with ${altid} is now pending! To transfer all points from ${altid} to ${user.id}, log in on the account '${altid}' and type /trivia mergescore ${user.id}`
      );
    }
  },
  mergescorehelp: [
    `/trivia mergescore [user] \u2014 Initiates a merge of another account's Trivia leaderboard score into your current account. Using this command again on the other account completes the merge. Attempts expire after 15 minutes.`
  ],
